...
function DesktopIcon({ appId, x, y }: Props) {
  ...
  return (
    ...
    >
      <img className="flex size-12 items-center justify-center text-3xl rounded-md" src={app.icon} draggable={false} />
      <span className="w-full truncate text-center text-[14px] tracking-wide">{app.title}</span>
    </button>
  );
}
